...
namespace AirStrikeKit
{
	...
	public class AIController : MonoBehaviour
	{
		...
		// list of Tags that AI will attack to
		public GameObject Target;
		// current target object.
		public float TimeToLock;
		// dularion to select the target
		public float AttackDirection = 0.5f;
		// direction that AI can see the target, aim (0 - 1) e.g. if  0  AI will see all direction.
		...
		// (0 - 100) e.g. if 100 this AI will always shooting.
		...
		public float AvoidChackDistance = 100;
		public float AvoidCheckOffset = 10;
		...
		Vector3 avoidDirection ()
		{
			...
			dirs [0] = this.transform.forward + this.transform.up; 
			dirs [1] = this.transform.forward - this.transform.up;
			dirs [2] = this.transform.forward;
			dirs [3] = this.transform.forward + this.transform.right;
			dirs [4] = this.transform.forward - this.transform.right;
			...
			for (int i = 0; i < dirs.Length; i++) {
				Debug.DrawLine (this.transform.position + (this.transform.forward * AvoidCheckOffset), this.transform.position + dirs [i].normalized * AvoidChackDistance, Color.white);

				if (Physics.Raycast (this.transform.position + (this.transform.forward * AvoidCheckOffset), dirs [i].normalized, out ray, AvoidChackDistance)) {
					Debug.DrawLine (this.transform.position + (this.transform.forward * AvoidCheckOffset), this.transform.position + dirs [i].normalized * AvoidChackDistance, Color.red);
					hited = true;
					// hit some obstacle
				}
			}
			...
			if (hited) {
				for (int i = 0; i < dirs.Length; i++) {
					if (!Physics.Raycast (this.transform.position + (this.transform.forward * AvoidCheckOffset), dirs [i].normalized, out ray, AvoidChackDistance)) {
						Debug.DrawLine (this.transform.position + (this.transform.forward * AvoidCheckOffset), this.transform.position + dirs [i].normalized * AvoidChackDistance, Color.green);
						return dirs [i];
						// find some free direction
					}
				}
